...
llama_pipeline = pipeline("text-generation", model=model, tokenizer=tokenizer, max_length=1000)
def generate_tax_expressions(data, llama_pipeline):
    task_prompt = """
    ...
    # Use the Llama pipeline to generate text
    response = llama_pipeline(full_prompt, max_length=1000, num_return_sequences=1)[0]['generated_text']
    ...
    try:
        ...
        result = [
            f"{r['description']} = {r['expression']} = {r['result']} {r['currency']}"
            for r in res_dict
        ]
        return "\n".join(result)
    except json.JSONDecodeError:
        return "Error: Unable to parse the generated JSON. Please try again."
...
def generate_tax_strategy(data, llama_pipeline):
    tax_results = generate_tax_expressions(data, llama_pipeline)
    ...
    # Use the Llama pipeline to generate text
    response = llama_pipeline(full_prompt, max_length=2000, num_return_sequences=1)[0]['generated_text']
